...
export interface TutorialStep {
  ...
  resolveSpotlight:
    ...
    | null; // null = keep previous spotlight
...
function neighbourhoodSpotlight({
  nodeToScreenCoords,
  selectedNodeId,
  neighbourIds,
}: SpotlightContext): SpotlightTarget | null {
  if (!nodeToScreenCoords || !selectedNodeId || neighbourIds.length === 0)
    return null;
...
export const TUTORIAL_STEPS: TutorialStep[] = [
  ...
  },
  {
    id: 'detail',
    prompt:
      'Click on "Retail And Wholesale Trade Managers" to see further details.',
  ...
  },
  {
    id: 'compare',
    prompt:
      'This view compares AI exposure, median wages, and skills between the two occupations — shared skills and skills to develop.',
    completionEvent: 'manual',
    resolveSpotlight: null,
  },
  {
  ...
  },
  {
    id: 'pathways',
    prompt:
      'Browse transition pathways — each card shows skill overlap, AI exposure, and wages for occupations you could move into.',
    completionEvent: 'manual',
    resolveSpotlight: null,
  },
  {
