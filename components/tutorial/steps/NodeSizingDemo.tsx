...
function scaledRadius(value: number) {
  return (NODE_RADIUS_BASE + value * NODE_RADIUS_SCALE) * SCALE;
}
...
export default function NodeSizingDemo() {
  ...
  useEffect(() => {
    ...
    const modeLabel = g
      ...
      .attr('opacity', 0)
      .text('Sized by: AI Exposure / Median Wage');

    let cancelled = false;

    // Single phase: scale uniform → varied sizes
    const t1 = d3.timeout(() => {
      if (cancelled) return;
      circles
        .transition()
  ...
  return (
    <svg
      ...
      style={{ height: 320 }}
      role="img"
      aria-label="Animation showing nodes sized by AI exposure or median wage"
    />
  );
}
